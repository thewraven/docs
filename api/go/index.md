...
- `Password : string` holds the password used for authentication (only used when using the v1 handshake protocol).
- `AuthKey : string` is used for authentication when using the v0.4 handshake protocol. This field is now deprecated.
- `Timeout : time.Duration` is the time the driver waits when creating new connections, to configure the timeout used when executing queries use WriteTimeout and ReadTimeout.
- `WriteTimeout : time.Duration` is the amount of time the driver will wait when sending the query to the server.
- `ReadTimeout : time.Duration` is the amount of time the driver will wait for a response from the server when executing queries.
- `KeepAlivePeriod : time.Duration` is the keep alive period used by the connection, by default this is 30s. It is not possible to disable keep alive messages.
- `TLSConfig : *tls.Config` holds the TLS configuration and can be used when connecting to a RethinkDB server protected by SSL.
- `HandshakeVersion: r.HandshakeVersion` is used to specify which handshake version should be used, this currently defaults to v1 which is used by RethinkDB 2.3 and later. If you are using an older version then you can set the handshake version to 0.4.
- `UseJSONNumber : bool` indicates whether the cursors running in this session should use json.Number instead of float64 while unmarshaling documents with
 interface{}. The default is `false`.
- `NumRetries : int` is the number of times a query is retried if a connection error is detected, queries are not retried if RethinkDB returns a runtime error.
- `InitialCap : int` is used by the internal connection pool and is used to configure how many connections are created for each host when the session is created. If zero then no connections are created until the first query is executed.
- `MaxOpen : int` is used by the internal connection pool and is used to configure the maximum number of connections held in the pool. If all available
 connections are being used then the driver will open new connections as needed, however they will not be returned to the pool. By default the maximum number of connections is 2.
- `DiscoverHosts: bool` is used to enable host discovery, when true the driver will attempt to discover any new nodes added to the cluster and then
